...
export default function AttendancePage() {
  ...
  const getAttendanceList = useCallback(() => {
    ...
    fetchAttendance(eventId)
      .then((response) => {
        ...
        setFilteredList(response.data.data.attendanceList);
      })
      .catch((error) => {
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(value);
    const filtered = Array.isArray(attendanceList)
      ? attendanceList.filter(
          (attendee) =>
            attendee.phoneNumber.toLowerCase().includes(value) ||
            attendee.email.toLowerCase().includes(value)
        )
      : [];
    ...
    <div className="min-h-screen p-6 bg-white">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-10">
          <div className="w-16 h-16 transition-colors duration-700 border-4 border-white rounded-full border-t-[#00B425] animate-spin"></div>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full overflow-hidden bg-white border border-gray-300 rounded-lg">
            <thead>
              <tr className="text-white bg-[#00B425] border-b border-gray-300">
            ...
            <tbody>
              {Array.isArray(paginatedList) &&
                paginatedList.map((attendee, index) => (
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded hover:cursor-pointer ${
                    currentPage === page
                      ...
                      : "bg-gray-300 text-gray-700 hover:bg-green-500 hover:text-white hover:cursor-pointer"
                  }`}
                >
                  {page}
                </button>
